...
import type { AddressInfo } from "../AddCep/AddCep";
...
function AddAddress() {
  ...
  const handleFinish = () => {
    ...
    const tokenValido = token && token !== '"undefined"' && token !== "undefined" && token !== "null";
      ...
      <div className="flex flex-col gap-2 text-center">
        ...
        {address && (
          <p className="text-light text-sm text-left">
            CEP: <span className="text-brand-primary font-bold">{address.cep}</span>
          </p>
        )}
      </div>
      <div className="flex flex-col gap-4">
        <Input type="text" name="street" id="street" label="" placeholder="Rua"
          value={address?.street || ""} fullWidth disabled />
        <div className="flex gap-4">
          <Input type="text" name="number" id="number" label="" placeholder="Número *"
            onChange={(e) => setHouseNumber(e.target.value)} disabled={loading} />
          <Input type="text" name="complement" id="complement" label="" placeholder="Complemento"
            onChange={(e) => setExtraInfo(e.target.value)} disabled={loading} />
        </div>
        <Input type="text" name="neighborhood" id="neighborhood" label="" placeholder="Bairro"
          value={address?.neighborhood || ""} fullWidth disabled />
        <div className="flex gap-4">
          <Input type="text" name="city" id="city" label="" placeholder="Cidade"
            value={address?.city || ""} disabled />
          <Input type="text" name="state" id="state" label="" placeholder="UF"
            value={address?.state || ""} disabled />
        </div>
      </div>
      <Button variant="primary" onClick={handleFinish} fullWidth disabled={loading}>
